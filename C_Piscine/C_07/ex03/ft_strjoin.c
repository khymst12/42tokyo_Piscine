...
/*   Updated: 2025/01/28 15:45:22 by kyamashi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_copy_to_end(char *dest, char *src)
{
	while (*src)
	{
		*dest = *src;
...
char	*ft_write(int size, int total_len, char **strs, char *sep)
{
	...
	while (i < size)
	{
		current = ft_copy_to_end(current, strs[i]);
		if (i < size - 1)
			current = ft_copy_to_end(current, sep);
		i++;
	}
	*current = '\0';
	return (result);
